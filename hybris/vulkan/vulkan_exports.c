...
void _vulkan_missing_symbol(const char *name)
{
    fprintf(stderr, "libhybris vulkan: %s is not provided by the Android driver\n",
            name ? name : "(unknown)");
    abort();
}
...
__asm__( \
    ...
    "    br x16\n" \
    "1:\n" \
    "    adrp x0, .L_hybris_missing_" #sym "\n" \
    "    add x0, x0, :lo12:.L_hybris_missing_" #sym "\n" \
    "    b _vulkan_missing_symbol\n" \
    ".size " #sym ", .-" #sym "\n" \
    ".section .rodata\n" \
    ".L_hybris_missing_" #sym ":\n" \
    "    .asciz \"" #sym "\"\n" \
    ".text\n" \
);
...
{
    for (const struct _vulkan_sym_entry *entry = __start_vulkan_syms;
         entry < __stop_vulkan_syms; entry++) {
        *entry->ptr = android_handle ? android_dlsym(android_handle, entry->name) : NULL;
    }
}
