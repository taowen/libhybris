...
static void hybris_apply_static_tls_locked(const struct hybris_promoted_tls* e)
{
    ...
    if (e->static_offset + e->segment_size > BIONIC_STATIC_TLS_SIZE) {
        fprintf(stderr, "HYBRIS: fatal: bionic static TLS overflow "
                        "(offset=%zu memsz=%zu, max=%d). Bump BIONIC_STATIC_TLS_SIZE in hooks.c.\n",
...
#ifdef WANT_ARM_TRACING
    _android_linker_init(sdk_version, __hybris_get_hooked_symbol, enable_linker_gdb_support, &tls_patcher_funcs, create_wrapper, wrappers_enabled());
#else
    _android_linker_init(sdk_version, __hybris_get_hooked_symbol, enable_linker_gdb_support, &tls_patcher_funcs);
#endif
