...
2026-09-08 G11 部分进展：标准 ICD 新增 XCB/Xlib surface 与 TAWC-DRI 0.3
原生 buffer 提交，独立 `tests/x11` 开发工具自建私有 Xwayland，复用隔离
compositor APK。高通 OnePlus 8T 与 Mali 的两种 API × 呈现/缺协议/超时
共 12 项 VVL+SyncVal 检查通过；呈现逐帧精确读回、两帧实屏及 release
序列共同验收。Wayland 完整 swapchain review 与四项 headless 回归两机通过。
...
2026-09-08 G07 BC4/BC5：默认关闭的图像兼容路径新增 UNORM/SNORM，
按归一化端点做整数有理插值，仅在最后舍入到 16 位。BC4 使用 R16、
BC5 使用 RG16，保留负值与缺失通道，仍不宣告 textureCompressionBC。
现有探针扩大为十二种格式和 32×32 对齐图像，跨越内部 64-set 池边界；
补原生 16 位采样、线性滤波、白色/透明黑边框与原始压缩字节对照。
最终 force 四入口和 missing 共十次图像执行完成 2,880 次完整回读，其中
960 次 BC4/BC5，全部零差异、零 VVL/SyncVal 错误。默认关闭的三种内核
路线双机另完成 1,152 次回读；十二格式的原生/透传不支持与 BC6/BC7
查询保持分别记录。构建、六个结果索引、首轮旧计数门槛失败及成本见
[BC4/BC5 记录](tests/baseline/bc-images.md#bc4bc5-extension-evidence-2026-09-08)。
BC6/BC7、BC1 RGB 边框/default-alpha、mutable/alias/跨队列、最大资源、
性能和 CTS 等仍开放；不据此关闭 G07。


2026-09-08 BC1 RGB 边框修复与保留失败：新增合法边框探针后，两机旧库均在
12 次 BC1 RGB UNORM/sRGB 回读出现 306 个 alpha 差异，VVL 错误仍为零。
现按物理设备能力选用原生 RGB8，GPU 四像素打包到三个字，不改变颜色精度；
Mali 的 force 四入口加 missing 共 1,440 次回读全部通过。红米缺少所需的
optimal RGB8/sRGB 支持，仍每次出现相同 306-word FAIL，没有跳过或降低门槛。
双机三种独立内核路线另通过 1,344 次回读；图像校验移至 bc_image_verify.h，
与 Vulkan 录制分开。构建、六个结果索引和限制见
[RGB8 记录](tests/baseline/bc-rgb8.md)。红米正确实现、custom border、通用
sampler/view、BC6/BC7 与其余 G07 条件仍未完成；该批只关闭已验证的 Mali 症状。

### G06 多动态 descriptor 的固定捕获证据（2026-09-08）

新增 `ubo-multi`：两个 set、四个动态 UBO、一个双元素数组，交错 binding
编号及逆序 layout 声明区分 set/binding/array 的偏移顺序。两阶段 shader
读取四个唯一标签和实际字段，256 像素逐一检查；捕获重建完整范围与
buffer/memory 创建绑定，在 draw 62 定位唯一改变的 set 0 / binding 3 /
element 1。三个捕获模式两台均通过，每台 24 个阶段退出码为零、18 份
完整图像一致。对保存证据的 22 个错误变体拒绝，四个合法顺序变体接受。

实际干净构建和真机矩阵：Redmi `20260908T121659-c81e25af` 为 28 PASS /
18 UNSUPPORTED，Mali `20260908T121659-7ea849f5` 为 43 PASS / 3 UNSUPPORTED；
UNSUPPORTED 的渲染入口未计覆盖。BC force 下两台普通/多 UBO validation
仍通过，Redmi `20260908T121721-c881dd08` 的 BC1 RGB 边框仍为真实 FAIL，
Mali `20260908T121723-31b91062` 的 288 次图像回读通过。详见
[多 descriptor 证据](tests/baseline/widget-multi.md)。

按职责拆出 widget pipeline 创建和 descriptor 审计，主探针从 758 行降至
677 行。此批未修改驱动兼容行为，未新增单元测试或通用框架；任意应用的
状态历史、对象 generation、非零 firstSet/部分重绑、template 数组与 WSI
lineage 仍未验收，G06/G07 保持开放。


2026-09-08 BC7：新增默认关闭的 GPU 解码及 RGBA8 图像接入，512 块独立
scalar/SIMD reference 覆盖八种模式、合法 partition、P-bit、rotation/index
selection；双机最终四路线内核共 2,112 次回读通过。Mali 四入口 force 与
missing 图像均通过，Redmi 保留 BC1 RGB 的 306 个差异与 BC7 sRGB 原生过滤
对 CPU golden 的 51 个差异，未放宽容差。BC7 的 BC/native、nearest、原始块
...
2026-09-08 packed 顶点：新增默认关闭的 A2R10G10B10 SNORM 静态管线 fallback，
