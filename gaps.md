...
已验证 headless 初步路径：标准 glibc loader → 可选 hybris ICD → vendor Vulkan HAL，在两台 Adreno 650 设备通过 8 项基础探针。标准 validation/capture 与 WSI 尚未完成。Android loader 已在 bridge 下游，必须验证双 loader 的 handle/dispatch 所有权，防止递归加载同名 libvulkan；Android 库用独立路径/命名空间明确解析。
...
| G04 / P0 | 标准 loader/layer/tool 接入 | 一个已知非法小测试被 validation 捕获；一个合法小测试零新增错误；完成一帧 capture/replay 且像素匹配，再扩大到应用。**部分落地**：可选 ICD 直接接 vendor HAL，标准 glibc loader 的 8 项 headless 用例在两台设备通过；不改写 dispatch header、不删创建链。尚无 glibc VVL 正负例或 capture/replay，WSI 未实现。此前手写 validation chain 的结果仍已撤回 |
