...
### G06 管线库顶点输入来源（2026-09-08）

把图形管线库的顶点输入解析拆到独立模块，在链接创建时保存来源与状态，
避免仅查看最终管线而漏掉库内动态输入声明。缺失库或多个顶点来源保留
不完整状态；链接后销毁库不丢失已保存证据。最终真机捕获
`20260908T190556-276762f8` 的 Turnip 50/50 次顶点输入完整，渲染和
50 张图像回放均 PASS；Mali `20260908T190556-bb8da642` 保持 27/27，
回放图像全部匹配，原始属性错误仍 FAIL。真实捕获副本的依赖缺失、冲突
