...
| G05 / P0 | 能力宣告与模拟实现脱节 | features/features2、properties/limits、extensions、format/image-format query 与 CreateDevice enable 路径一致；保留原始/有效能力差异及原因；不通过删整个 pNext 重试。**部分落地**：baseline `caps` 核对未广告 feature / 未知扩展的精确拒绝错误，并检查未启用扩展的 GDPA 返回 NULL；已保存同设备 native/hybris/ICD 的 325 个具名查询值与差集：174 个 core feature/limit/sparse 值、设备标识、设备扩展及十种格式查询。两台设备前端无差异，直接 HAL ICD 有六项 buffer/present 扩展差异；不能按全栈等价解释。尚无 features2 全链、全部格式创建验证和兼容变换原因记录 |
