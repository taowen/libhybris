...
| G03 / P0 | 多线程/多 context/多 device 与对象生命周期 | 并行 create/destroy、二次 init/dlopen、回调、线程 TLS、handle 重用有回归；对象 state 按 generation 识别，不能用进程全局单一 current device。**部分落地**：baseline `life` 覆盖双 device、destroy/recreate、二次 dlopen、两线程 create/destroy。`unload` 在 `20260906T230438-cbc4c7cd` 上 native/hybris 都能完成最后一次 frontend `dlclose` 并正常退出；当前改为链接时设置 `DF_1_NODELETE`，使 `libhybris-common` 从加载起常驻；Android hook 指针和 TLS cleanup 回调可能比 frontend 活得更久。此策略保留 common 代码/状态，但不保证回调与对象已清理，也不阻止进程退出 finalizer。这不是 Android driver 卸载，也没有 generation 对象表或 GLES 多 context |
