...
- **捕获资源**：GFXReconstruct 导出 draw-time descriptor、完整 272B UBO、attachment 前后图，核对 update/bind 与 set/buffer/range、attachment 与 copy 源 image。两台设备普通负对照的首次像素分叉是 draw 60；动态单 binding 关联 base＋dynamic offset 与 vertex/fragment dump，首次分叉是 draw 61。未捕获、捕获和回放的完整图像一致；篡改绑定偏移或只保留 base 的离线证据被拒绝。
