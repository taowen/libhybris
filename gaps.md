...
- ES3 专用功能、Vulkan texture/compute、GLES 并行绘制与共享对象在仍被引用时删除尚未测；widget shader 和 device/fence 双线程不足以代表 API feature level。
