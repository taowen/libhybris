...
/* Blender 4.3 vk_backend.cc::missing_capabilities_get / is_supported. */
int blender_vk_probe(void)
{
  ...
  for (uint32_t i = 0; i < count; ++i) {
    ...
    p_vkEnumerateDeviceExtensionProperties(devices[i], NULL, &ext_count, NULL);
    VkExtensionProperties *ext = calloc(ext_count, sizeof(*ext));
    if (ext_count && !ext) {
      p_vkDestroyInstance(instance, NULL);
      return 2;
    }
    if (ext_count) p_vkEnumerateDeviceExtensionProperties(devices[i], NULL, &ext_count, ext);
    int swapchain = 0, dyn_ext = 0;
