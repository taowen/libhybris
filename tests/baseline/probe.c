...
int main(int argc, char **argv) {
  ...
  if (!strcmp(mode, "version"))
    ...
    rc = scaled_vertex_probe(strstr(mode, "validation") != NULL,
      strstr(mode, "gdpa") ? 1 : strstr(mode, "elf") ? 2 : strstr(mode, "linked") ? 3 : 0, strstr(mode, "spec-direct") ? 8 : strstr(mode, "spec") ? 7 : strstr(mode, "matarray") ? 6 : strstr(mode, "nested") ? 5 : strstr(mode, "array") ? 4 : strstr(mode, "matrix") ? 3 : strstr(mode, "literal") ? 2 : strstr(mode, "multi") != NULL);
