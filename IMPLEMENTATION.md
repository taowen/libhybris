...
| G06 | Partial | Fixed captures now reconstruct four dynamic UBOs across two sets and an array and identify the changed descriptor/first divergent draw. The [desktop GL capture](tests/desktop-gl/capture.md) also indexes recorded vertex inputs and submit references; indirect arguments and linked pipeline-library state remain undecoded. General history and runtime resource generations remain open; see the multi-descriptor evidence. |
